...
echo "::group::Check Servatrice database schema version"
...
echo "Latest migration: $latest_migration ($old_ver → $new_ver)"
...
if ((schema_ver != new_ver)); then
  echo "Schema version $schema_ver does not match new version $new_ver"
  exit 1
fi
