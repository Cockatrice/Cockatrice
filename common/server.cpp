...
void Server::setSettingsInterface(Server_SettingsInterface *_settingsInterface)
{
    connect(this, SIGNAL(endSession(qint64)), _settingsInterface, SLOT(endSession(qint64)));
    settingsInterfaces.insert(QThread::currentThread(), _settingsInterface);
}

Server_SettingsInterface *Server::getSettingsInterface() const
{
    return settingsInterfaces.value(QThread::currentThread());
}
