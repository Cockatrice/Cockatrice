...
void ZoneViewZone::initializeCards(const QList<const ServerInfo_Card *> &cardList)
{
    if (!cardList.isEmpty()) {
        ...
        for (int i = 0; i < number; i++) {
            ...
            addCard(new CardItem(player, card->getName(), card->getProviderId(), card->getId(), revealZone, this), false, i);
        }
        reorganizeCards();
    }
}
...
void ZoneViewZone::zoneDumpReceived(const Response &r)
{
    ...
    for (int i = 0; i < respCardListSize; ++i) {
        ...
        auto *card = new CardItem(player, cardName, cardProviderId, cardInfo.id(), revealZone, this, this);
        cards.insert(i, card);
    }
    reorganizeCards();
