...
void DeckEditorDatabaseDisplayWidget::updateCard(const QModelIndex &current, const QModelIndex & /*previous*/)
{
    ...
    if (!current.model()->hasChildren(current.sibling(current.row(), 0))) {
        emit cardChanged(CardDatabaseManager::getInstance()->getPreferredCard(
            {CardDatabaseManager::getInstance()->getCardInfo(cardName)}));
    }
}
...
ExactCard DeckEditorDatabaseDisplayWidget::currentCard() const
{
    ...
    return CardDatabaseManager::getInstance()->getPreferredCard(
        {CardDatabaseManager::getInstance()->getCardInfo(cardName)});
}
