...
void LineEditCompleter::focusOutEvent(QFocusEvent *e) {
    ...
    if (mentionCompleter->popup()->isVisible()) {
        ...
        insert(mentionCompleter->completionModel()->index(mentionCompleter->popup()->currentIndex().row(), 0).data().toString() + " ");
    ...
    if (cardCompleter->popup()->isVisible()) {
        ...
        setText(textValue.left(leftShift));
        //Insert highlighted line from popup
        insert(cardCompleter->completionModel()->index(cardCompleter->popup()->currentIndex().row(), 0).data().toString() + " ");
        //Set focus back to the textbox since tab was pressed
        setFocus();
    }
}
...
void LineEditCompleter::processMention() const {//Set new completion prefix
    ...
    //Draw completion box
    QRect cr = cursorRect();
    cr.setWidth(mentionCompleter->popup()->sizeHintForColumn(0) + mentionCompleter->popup()->verticalScrollBar()->sizeHint().width());
    mentionCompleter->complete(cr);
...
void LineEditCompleter::processCard() const {
    ...
    //Draw completion box
    QRect cr = cursorRect();
    cr.setWidth(cardCompleter->popup()->sizeHintForColumn(0) + cardCompleter->popup()->verticalScrollBar()->sizeHint().width());
